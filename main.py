...
def get_num_adjacent_mines(x, y, mineList):
...
def generate_mines(numMines, initialX, initialY):
    generatedMines = []
    while len(generatedMines) < numMines:
        ...
        if newMine not in generatedMines and \
            ...
            get_num_adjacent_mines(initialX, initialY, [newMine]) == 0:
...
def main():
    ...
    try:
        ...
        if not is_valid(selectedX, selectedY):
            raise ValueError("Invalid input")
        ...
        # If needs new game
        if not mines:
            mines = generate_mines(40, selectedX, selectedY)
            actionsEnv = "encrypt=true"
        overwrite_close(file, json.dumps(mines))
        ...
        if "game_flag" in sys.argv[1]:
            ...
            while len(toSet) > 0:
                ...
                numAdjacentMines = get_num_adjacent_mines(cur[0], cur[1], mines)
                fieldArray[cur[1]][cur[0]] = ADJACENT[numAdjacentMines]

                for delta in deltas:
                    newPoint = (cur[0] + delta[0], cur[1] + delta[1])
                    if is_valid(newPoint[0], newPoint[1]) and numAdjacentMines == 0:
                        toSet.add(newPoint)
        ...
        overwrite_close(file, json.dumps(fieldArray))
        ...
        table = table.replace("FIELD_GOES_HERE", render_markdown(fieldArray))
        file = open(filenameToWrite, "r+", encoding="utf8")
        overwrite_close(file, table)
        ...
        if gameOver:
            ...
            markdown = render_markdown(json.load(file))
            file.close()
    ...
    # Some errors can be attributed to improper user input
